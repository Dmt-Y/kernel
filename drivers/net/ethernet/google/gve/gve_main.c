...
static int gve_verify_driver_compatibility(struct gve_priv *priv)
{
	...
	*driver_info = (struct gve_driver_info) {
		.os_type = 1, /* Linux */
		.os_version_major = cpu_to_be32((LINUX_VERSION_CODE >> 16 & 0xff)),
		.os_version_minor = cpu_to_be32((LINUX_VERSION_CODE & 0xff)),
		.os_version_sub = cpu_to_be32((LINUX_VERSION_CODE >> 8 & 0xff)),
...
{
#if 0 /* xdp_features doesn't exist in SLE12-SP5 */
	if (priv->queue_format == GVE_GQI_QPL_FORMAT) {
