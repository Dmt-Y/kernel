...
static int
iscsi_set_param(struct iscsi_transport *transport, struct iscsi_uevent *ev, u32 rlen)
{
...
iscsi_if_recv_msg(struct sk_buff *skb, struct nlmsghdr *nlh, uint32_t *group)
{
	...
	case ISCSI_UEVENT_SET_PARAM:
		err = iscsi_set_param(transport, ev, rlen);
		break;
	...
	case ISCSI_UEVENT_SEND_PDU:
		if ((ev->u.send_pdu.hdr_size > rlen) ||
		    (ev->u.send_pdu.data_size > (rlen - ev->u.send_pdu.hdr_size))) {
			err = -EINVAL;
			break;
		}
