			...
			u64 len)
{
	...
	ret = qgroup_reserve_data(BTRFS_I(inode), reserved_ret, start, len);
	if (ret <= 0 && ret != -EDQUOT)
		return ret;

	ret = try_flush_qgroup(BTRFS_I(inode)->root);
	if (ret < 0)
		return ret;
	return qgroup_reserve_data(BTRFS_I(inode), reserved_ret, start, len);
}
